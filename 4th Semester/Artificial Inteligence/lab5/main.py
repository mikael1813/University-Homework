...
class AntColonyOptimizer:
    ...
    def fit_dinamic(self, map_matrix, iterations=100, mode='min', early_stopping_count=50, verbose=True):
        """
        Fits the ACO to a specific map.  This was designed with the Traveling Salesman problem in mind.
        :param map_matrix: Distance matrix or some other matrix with similar properties
        :param iterations: number of iterations
        ...
        for i in range(iterations):
            ...
            paths = []
            path = []
            posx1 = random.randint(0, len(self.map)-1)
            posx2 = random.randint(0, len(self.map)-1)
            posy1 = random.randint(0, len(self.map)-1)
            posy2 = random.randint(0, len(self.map)-1)
            xx = random.randint(0, 40)
            yy = random.randint(0, 40)
            self.map[posx1][posy1] = xx
            self.map[posx2][posy2] = yy
...
problem1 = readNet("hard.txt")
problem = np.array(problem1)
optimizer = AntColonyOptimizer(ants=100, evaporation_rate=.1, intensification=1.5, alpha=2, beta=4,
                               beta_evaporation_rate=0, choose_best=.1)

#best = optimizer.fit(problem, 100)
best = optimizer.fit_dinamic(problem, 100)
optimizer.plot()
